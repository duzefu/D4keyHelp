# D4keyHelp
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

D4KeyHelp是一个有图形界面，可自定义配置的暗黑4鼠标宏工具。

| 浅色模式 | 深色模式 |
| :---: | :---: |
| ![mainPage](mainPage.png) | ![mainPage-dark](mainPage-dark.png) |

界面右上角可切换浅色/深色模式，运行中开始按钮会变为红色的“停止”。

运行代码请使用最新版的AHK v2.0软件!并非常见的V1.3的版本,你也可以直接在 [Releases](https://github.com/duzefu/D4keyHelp/releases) 处下载 `D4keyHelp.exe` 而不安装 AHK。

默认 F1 启动/暂停战斗宏，可在界面底部配置为键盘热键或鼠标中键/侧键（不含左右键）

**F3自动嬗变**：在赫拉迪姆魔盒界面扫描右下11列x3行物品格（单次截图+对截图取色），按边框 orange/red 光效识别传奇装备并跳过黄色稀有、蓝色魔法与空格子，逐个右键传奇物品并点击嬗变/重塑. 运行中再按 F3 可取消

**升级黄装**: 勾选了之后会自动给黄装添加词条(如果这个黄装已经是四词条的话，它会随机重置一个词条)后升级成传奇物品后再进行善变。


tab键查看地图的时候会暂停宏(但是有概率不起效果)

**维持BUFF**的功能,目前仅支持2K分辨率(其他分辨率我没有测试过,请自行测试,理论上是可用的。)+ 暗黑4内设置技能框在正中下方的UI设置

**鼠标自移动**是专门为了罗盘使用的，配合强制移动，它会让你在罗盘里面绕着圆圈走。自动捡魂魄，也自动打怪。只要在组队里，有人替你选贡品的话，就可以全程挂机。

**罗盘专用**这个按钮是原本设计成全自动选贡品的，就是每打怪一分钟 他都会点击脚下的那个贡品，然后自动开始下一关。但这种只适合不需要动来动去 并且 全程状态免疫的BD, 不然会有问题.

![示意图](0c3625c4-745f-49c4-b506-5c189ef1f439.png "示例图")

---
原本是打算改改 @https://github.com/WeijieH/D3keyHelper 用的,但是发现AHK1.3不论我怎么改在暗黑4都有奇怪的卡鼠标的问题(按F1暂停有概率鼠标卡住不动,宏也不会停止),改成AHK2.0之后问题消失了,但是也没有兴趣再移植老沙宏到AHK2.0了.随便写了个,就用这个吧

---
杂谈: 
1. 本脚本含有点位取色功能(用于 提供维持BUFF功能), 不知道会不会封号
2. 仅我个人而已, 我用了好几个赛季了从没被封过(steam国际服), 导航也是用的飞起. 感觉没那么容易封号
3. 不过这种鼠标宏部分在很多情况不如手柄宏, 用过steam 手柄宏的应该懂我在说什么. 所以也而没有那么有用

---

## Star History

<a href="https://star-history.com/#duzefu/D4keyHelp&Date">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=duzefu/D4keyHelp&type=Date&theme=dark" />
    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=duzefu/D4keyHelp&type=Date" />
    <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=duzefu/D4keyHelp&type=Date" />
  </picture>
</a>
